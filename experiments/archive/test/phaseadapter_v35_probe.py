...
class PhaseAdapterConfig:
    ...
    inject_last_n_blocks: int = 2
    inject_mode: str = "adaln"  # adaln | adaln_gated | adaln_hybrid | input_add | cross_attn
...
class _PhaseAdapterInjector:
    ...
    def _patch(self) -> None:
        for i, block in enumerate(self.blocks):
            ...
            def patched(x, c, cos_emb, sin_emb, _orig=orig, _i=i):
    ...
    def set_cond(self, cond_list: list[torch.Tensor] | None) -> None:
        self.ctx["cond"] = cond_list
...
def _select_blocks(lm: ADDSELightningModule, inject_last_n: int) -> list[ADDSEDiTBlock]:
    all_blocks = lm.model.time_dit.blocks  # type: ignore[attr-defined]
    n = max(1, min(int(inject_last_n), len(all_blocks)))
    return list(all_blocks[-n:])
...
def _evaluate(
    ...
    for noisy, clean, _ in loader:
        ...
        if use_adapter:
            ...
            cond_list = [cond_tokens for _ in injector.blocks]
            injector.set_cond(cond_list)
            try:
...
def run_probe(cfg: PhaseAdapterConfig) -> dict:
    ...
    _freeze_backbone(lm)
    blocks = _select_blocks(lm, cfg.inject_last_n_blocks)
    ...
    for noisy, clean, _ in loader:
        ...
        cond_full = adapter(phase_feat, target_len=y_tok.shape[-1])
        cond_tokens = cond_full
        cond_list = [cond_tokens for _ in blocks]
    ...
    report = {
        "meta": {
            ...
            "inject_last_n_blocks": int(cfg.inject_last_n_blocks),
        },
        "train_log": train_log,
